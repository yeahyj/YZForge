...
- ContentPack manifest 必须带 generator version 和 content hash。
...
生成的 TS 文件不写文件头；通过其固定目录和 `*.generated.ts` 命名识别。ContentPack manifest 仍通过 hash 检查是否被手改。
...
- root `tsconfig.json` 必须保持可迁移：不设置已弃用的 `baseUrl`，`moduleResolution` 使用 `bundler`，`paths` 目标使用显式相对路径，例如 `db://assets/*` 指向 `./assets/*`，不能写入项目根绝对路径。
