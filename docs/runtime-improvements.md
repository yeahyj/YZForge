...
| 页面内部 await 返回清理可能等待自身回调       | show.ui.back / show.dismiss / show.finish 发出请求；外部用请求 completed 或 handle.result 等待 |
...
1. **返回导航**：页面内部改用 `show.ui.back()`。外部原来 `await ui.back()` 若需要等物理完成，改为 `await ui.back().completed`。不要在当前页的受管任务里等待 `.completed`，否则又会等待自己。
