...
初始化和普通调用超时或取消后，调用方会立即收到失败；App 清理仍等待已经开始的底层工作结束，再销毁 SDK，避免迟到的资源泄漏。适配器必须让底层工作最终结束，不能遗留永不完成的 Promise。

需要主动中止底层工作的适配器可实现 `stop()`：App 取消时按逆序发出各适配器的停止请求，允许异步收尾；所有调用和停止请求完成后，才逆序执行 `dispose()`。`stop()` 保留仍被在途调用使用的共享状态，`dispose()` 负责最终释放；不要让在途调用依赖 `dispose()` 才能结束。内置小游戏适配器通过 `stop()` 清理正在展示的广告，页面取消仍只结束该页面的等待。停止或销毁失败都会传递到关闭流程。
...
返回的渠道适配器只委托需要的能力，不用 `{ ...platform }` 复制平台的 `initialize` / `stop` / `dispose`。第三方实例由 `use` 管理，不再在渠道适配器中重复初始化或销毁它。工厂构造期间应只分配可回收对象，SDK 的异步启动放到 `initialize`。
