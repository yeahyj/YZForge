...
12. 普通远程资源按需加载。整包预下载是显式功能，不因校验而默认读取整个包。
...
- 当前模块/资源包按需读取相关索引；`resolve` 是异步 API，冷缓存时会准备必要 Bundle 配置并读取索引。默认按需交付配置下不会因此下载目标图片或整个资源包；若主动选择 Zip 等整包交付，实际下载下界由该交付方式决定，面板必须标明。
...
3. 选择主键和目标：公共包、模块默认包、指定资源包，或按声明的列分片。
...
| A09 | 未读取目标资源包索引时查名、再实际加载 | 默认按需方案只读必要配置/索引；实际加载才读取目标及其依赖，其他资源包不提前访问 |
