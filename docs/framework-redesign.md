...
>
> 当前状态：设计提案。本文中的目录、类型和 API 均为新方案示意，尚未实现或完成平台验证。现有实现、历史设计和测试通过记录不能作为新方案已可用的证明。
...
| Bundle | 一组实际打包与交付的资源；一个模块有默认资源包，可以增加章节资源包 |
...
11. 保留章节内容独立分包的能力，直接使用 Cocos Bundle、统一配置入口和 Scope；不另设 ContentPort、PackRef/PackAssetRef、Pack manifest 和独立内容包生命周期。附加 Bundle 按实际需求创建。
12. 普通远程资源按需加载。完整章节预下载是显式功能，不因校验而默认读取整个包。
...
这里精简的是额外框架机制，并未取消“复用代码”或“把章节资源独立交付”的需求。目录改名不构成架构简化；验收应看新增能力是否仍要求专属注册、安装声明、引用类型和生命周期。如果这些手续仍在，即使换了目录名也没有完成本方案的精简。
...
| 游戏分模块分包 | 逻辑模块 + 默认资源包 + 可选附加资源包；代码交付按平台配置 | 首包内容、模块依赖及远程资源归属可从产物确认 |
...
config-source/
  tables.json                     # 表来源、sheet、目标模块/包与分片映射
  common/
  battle/
project-settings/
  framework.json                  # 命名规则、公共资源登记、UI/音频默认值、构建方案
.yzforge/                         # 本机缓存、变更记录、回收区；不进入 assets
```

目录按实际需要创建，空项目不预建所有业务文件夹。`code/services` 只是推荐位置，普通业务对象也可直接放在模块内。

默认模块只有 `code` 和 `res` 等基本结构。确实需要独立交付森林章节时，才由面板增加下面的可选目录；每个子目录直接配置为一个 Cocos Asset Bundle：

```text
modules/battle/
  bundles/                         # 仅存在附加 Bundle 时创建
    forest/
      levels/
      audio/
      config/
...
| 声明 | 唯一事实 |
| --- | --- |
| `module.json` | 模块 id、依赖、默认包及附加包、UI 注册项、音频条目、动态资源收录规则和名字到 UUID 的登记 |
| `config-source/tables.json` | 工作簿/Sheet 到 tableId 和目标包的映射、主键/索引/约束、可选分片规则 |
| `project-settings/framework.json` | 全局约定、公共资源 sharedAssets 登记与平台构建方案 |
...
- Cocos 类名全项目唯一，创建工具生成稳定类名，类名不承载 UI 分类语义。
...
```json
{
  ...
  "bundles": {
    "default": { "id": "m-battle", "root": "res" },
    "forest": { "id": "battle-forest", "root": "bundles/forest" }
  },
  "assets": {
    "battle/default/prefab/reward-popup": {
      "type": "Prefab",
      "uuid": "由编辑器记录的真实 UUID"
    }
  },
  "views": {
    "reward": {
      "prefab": "battle/default/prefab/reward-popup",
      "kind": "popup",
      "cache": "none",
      "duplicate": "reject"
    }
...
此示例只展示声明结构；实际 Schema 必须拒绝占位 UUID，面板必须从 AssetDB 选择真实对象。

资产登记是逻辑 id→UUID/子资源身份的唯一事实。views 只保存 Prefab 的逻辑 id 与 UI 选项，音频条目同样只引用 AudioClip 逻辑 id；不再各自保存一份 UUID 或物理路径。类型化 View 引用、地址和依赖都是派生产物，不能反向写成第二份源声明。
...
| 物理地址 | Bundle `m-battle`，路径 `icons/coin/spriteFrame` | 随 AssetDB 和实际构建结果更新 |

逻辑标识格式为 `<module>/<group>/<kind>/<name>`：`default` 是模块默认组，`forest` 等是可选内容组；sprite、texture、prefab、audio 等是明确类型分类。`shared` 保留给公共资源，例如 `shared/default/sprite/coin`，不能再创建同名业务模块。各段使用小写字母、数字和短横线，首字符为字母；生成 TypeScript 字段时转成合法标识符并检查冲突。中文用于显示名，原文件名不必强行改成英文。

内容组是查名范围，不等于不可变的物理 Bundle 路径。资源移到共享 Bundle 后可以保持原逻辑标识，新的索引记录负责指向正确交付位置并更新包依赖。

同一图片的 SpriteFrame 和 Texture2D 是不同类型条目，例如 `battle/default/sprite/coin` 与 `battle/default/texture/coin`。选择图片时面板默认收录 SpriteFrame，其他类型按需要勾选；不能把“图片名字”自动猜成任何一种图片相关类型。
...
- 跨组或跨模块使用完整逻辑 id，或对应的生成引用，例如 `BattleForestRes.sprite.coin`。
- 没有默认模块上下文的 App 资源入口必须显式选择 namespace，或使用完整逻辑 id/生成引用。
- 允许不同组各有 coin；同一组同一类型的两个 coin 必须在面板改成不同逻辑名，例如 gold-coin、silver-coin。
...
```ts
// battle/generated/resources-default.ts，生成产物示意
...
面板列出模块与公共资源中的可加载候选。开发者可以选择“收录这个动态资源目录”，随后目录内新增资源自动进入检查；也可以单项收录或排除，不要求逐个手写路径。

生成过程：

1. 从 Creator AssetDB 读取资源类型、UUID、子资源和包归属，应用收录范围与类型规则；父目录扫描排除独立子 Bundle、工具生成的索引本身和未支持类型。
2. 对新资源建议逻辑名；无冲突时写入所属 module.json 的 assets 登记，公共条目写入项目设置中的 sharedAssets 登记。
3. 登记把逻辑 id 绑定到真实 UUID/子资源身份，之后更新按 UUID 对应，不按文件名重新猜身份。
4. 生成按命名空间拆分的 TypeScript 引用和地址索引，排序稳定，只更新变化的命名空间。
5. 构建后核对 Bundle 输出和可加载地址；被过滤掉、类型不匹配或地址不可解析的条目使构建失败。

名字登记由面板维护并进入版本管理，不能只存在于本机缓存。它增加的是资源查询所需的 id→资源映射，直接使用已有声明，不再创建一套 Pack manifest 或扩展安装文件。

移动或重命名源文件而 UUID 保持时，只改变物理地址，逻辑 id 和业务字段保持不变。资源删除后登记标为失效；同路径重新导入但 UUID 改变时，需在面板明确选择“替换对应资源”，不能自动把旧名字绑到另一个文件。

逻辑重命名时，更新静态代码引用和配置引用；确有兼容旧内容需求时可保留一个显式旧名别名。别名只指向规范 id，不链式转发、不自动创建，过渡结束后经引用检查移除。

取消清单收录与删除源资产是两个操作。取消条目会检查代码、UI、音频和表引用；删除资产还要检查 Creator 原生序列化依赖。

#### 5.3.6 清单按需读取，不把全部内容拉进首包

编辑器可以展示全项目清单；运行时不内置一份所有章节的完整路径字典。
...
- 当前模块/资源包按需读取相关索引；`resolve` 是异步 API，冷缓存时会准备必要 Bundle 配置并读取索引。默认按需交付配置下不会因此下载目标图片或整个章节；若主动选择 Zip 等整包交付，实际下载下界由该交付方式决定，面板必须标明。
...
View 注册项与音频条目继续保存各自业务选项，底层资产只引用同一资源目录中的逻辑 id。面板选择 Prefab/AudioClip 时，先按 UUID/类型查找已有规范条目，没有时才创建，再写入业务引用；更新这组声明遵守第 14.5 节的恢复规则。替换或移动资产只更新资产登记与派生地址，View/音频面板的 UUID、路径信息仅供读取，禁止另存覆盖值。
...
资源页提供“全部资源”和“动态清单”两个视图。全部资源用于检查资产及引用关系；动态清单只展示已登记的加载入口。列表支持按名字、类型、模块、内容组、Bundle、静态引用/动态入口标记和状态筛选，图片可预览，其他类型显示图标与基础信息。
...
操作包括：选择扫描目录、收录/排除、编辑逻辑名、替换源资源、定位 AssetDB、查看使用方、复制完整名字、复制类型化加载代码、刷新和检查。

未读取索引的运行时调试视图应明确显示“未解析”，不能为了展示所有资源状态而加载所有章节索引。未知名字、重名、类型不符、索引版本不兼容分别报告，不静默返回 null 或任意候选。
...
编辑器展示两个独立状态：**已发现序列化引用**由依赖分析得出，**收录动态清单**由目录规则或单项登记决定。不增加要求用户手填的 static/dynamic 枚举。
...
静态依赖的加载时机随入口而定：远程关卡 Prefab 的固定贴图不等于必须随启动场景加载。反之，启动场景已经直接引用的资源，不会因为勾选“动态清单”就自动变成延迟加载。构建与首包分析必须展示实际依赖链。[Cocos 资源与依赖加载](https://docs.cocos.com/creator/3.8/manual/zh/asset/dynamic-load-resources.html)
...
目录按模块、用途和交付边界组织，不强制新建 static/ 和 dynamic/ 两个目录，不为两种用法复制资产，不创建 StaticAssetManager/DynamicAssetManager。确实便于团队管理时可以约定一个图标扫描目录，但目录名只定义收录范围。
...
| 同一 namespace 同一类型有两个 coin | 不允许 | 编辑期报出两个源位置，要求设置不同逻辑名 |
...
| 模块 `res` | 一个默认资源 Bundle | 本地、远程、平台资源分包 |
| 模块可选的 `bundles/<id>` | 独立无代码资源 Bundle | 章节、活动或可独立下载的地图集合 |
| shared/res | 公共资源 Bundle | 根据启动需要本地或远程，避免成为巨大兜底包 |
...
代码入口不能序列化引用默认包或章节包的全部内容；它仅保存入口组件。模块代码 Bundle 的构建顺序与资源 Bundle 的依赖关系由适配器明确配置，确保 UI Prefab 的脚本引用不会让代码重复进入远程资源包。公共组件只放公共代码侧，不能让章节包反向成为代码依赖。
...
- 启动场景、常驻 UI、公共静态字段直接引用远程章节 Prefab，导致提前包含依赖。
- 公共合同通过普通 import 或 barrel 间接引用模块实现。
- 资源包包含脚本，或章节 Prefab 引用了其模块未交付的脚本。
...
默认按章节/美术主题分包。纯配置关卡可以共用一个包；资源独立、体积大且需要独立更新时才按关拆包。
...
| 部分 | 入口 | 职责 |
| --- | --- | --- |
| 制作端 | Creator 工作台、工具 API、CLI | 读取 Excel/CSV、校验、选择分包、生成数据与 TS、预览差异 |
...
3. 选择主键和目标：公共包、模块默认包、指定章节包，或按列分片。
...
源数据第一版支持 `.xlsx` 和 UTF-8 `.csv`；CSV 使用相同表头约定，并按 CSV 的引号、逗号和换行规则解析，不能用简单 split 代替。旧 `.xls`、宏工作簿和额外来源适配不列为首版保证。Excel 解析依赖仅存在于工具端，不进入小游戏/原生包。
...
| 2 类型 | int | string | int | enum<normal,elite,boss> | int[] | asset<SpriteFrame>? |
| 3 默认值 | | | 1 | normal | [] | |
...
| `enum<normal,elite,boss>` | `elite` | 字符串联合类型 / string | 必须等于声明成员，大小写不自动纠正 |
...
- 公式单元格拒绝导入并要求转换为值；有数据区的合并单元格拒绝导入。日期时间用明确时区的 ISO 8601 文本，日期可用 YYYY-MM-DD；不猜 Excel 日期序号或本地时区。
...
表头负责字段名、类型、默认值和注释；`config-source/tables.json` 保存 source/Sheet→tableId/目标包、主键、索引、约束与分片映射。面板编辑这份声明，不另建一套手写字段 schema。
...
不默认生成“所有数据写成 TS 常量”的模式，否则可能把章节数据随代码带入首包；也不同时维护 JSON、TS 数据、二进制三份可编辑真相。Cocos 可以将 JSON 作为 JsonAsset 动态加载，配置系统在它之上做数据合同与查询封装。[Cocos JSON 资源](https://docs.cocos.com/creator/3.8/manual/zh/asset/json.html)
...
```text
config-source/battle/Monster.xlsx                  手写源数据
config-source/tables.json                          面板维护的映射/规则
assets/game/modules/battle/generated/config/
  Monster.types.ts                                行类型、主键类型、索引参数类型
  Monster.table.ts                                TableKey 与数据校验合同
  tables.ts                                       模块内便捷导出 BattleTables
assets/game/modules/battle/res/config/
  monsters.json                                   默认包的数据
assets/game/modules/battle/bundles/forest/config/
  levels.json                                     仅当关卡表配置了森林分片
```

类型文件只用 type import 引用其他合同，字段含注释，数组和结构递归只读。例如：
...
`TableKey<Row, PrimaryKey, Indexes>` 是 defineTable 返回的类型化表描述。它包含表身份及轻量数据合同，不包含所有行数据，不静态 import JsonAsset，也不手写固定 CDN 地址。模块内可生成 `BattleTables.monsters = MonsterTable` 的便捷聚合；跨模块或首包对单表有需求时直接 import 对应 .table.ts，纯类型需求使用 import type，避免无意带入全部表描述。
...
资源索引的表路由可以随兼容内容版本变化，TableKey 不复制物理路径。公共数据放公共包；不会为了让外键通过而复制进每个章节。代码侧只保留所需表合同，远程包里只有数据与资源。
...
强外键默认允许同包表、已声明的公共表和模块默认表。跨兄弟章节的强外键默认拒绝；导航等用途可以使用普通 ID 字段，或给 ref 字段显式配置 soft 模式。soft 仍能在构建时检查已知记录存在，但允许目标章节尚未加载。强弱外键都不会在读表时自动递归加载目标表；需要联合查询的业务显式 loadMany 或分别加载所需表。
...
运行时只加载所需表与分片，数组中的一行仍以所在 JSON 文件为最小读取粒度。若单表过大，应先按章节/用途分片；不承诺 get(101) 只从服务器下载第 101 行。监测下载字节、JsonAsset 占用、校验/冻结/建索引耗时和首关延迟，再决定是否需要增量解析或二进制格式。
...
面板名称“YZForge 工作台”。左侧项目树显示公共资源、模块及附加资源包；右侧为上下文操作，不要求用户记忆 Workflow 枚举和内部路径。
...
| 资源清单 | 全部资源/动态清单视图、收录目录、编辑名字、查引用、复制加载代码 | 静态引用与动态入口标记、逻辑 id、类型、真实地址、冲突和导出状态 |
...
| 创建项 | 用户填写 | 自动完成 |
| --- | --- | --- |
| 模块 | id、显示名、依赖 | module.json、工厂模板、public.ts、默认资源目录及 Bundle 配置，空的模块资源登记 |
...
| 配置导入 | 工作簿/Sheet、tableId、目标包，按需配置索引/约束 | 导入映射、分包 JSON、行类型、表描述/校验合同、表路由与报告 |
| 音频条目 | AudioClip、逻辑名、类别 | 资源目录引用与音频目录条目；不重复复制已存在资源 |
| 动态资源入口 | 已有资源或扫描目录、内容组，冲突时调整名字 | UUID 登记、类型化引用、按组索引、导出检查；不为收录复制素材 |
...
- 源文件移动/改名保持动态逻辑 id；修改逻辑名需要同时处理生成字段、代码和配置引用。替换源 UUID 是明确的资源替换操作，不能由同名文件自动触发。
...
| 配置导入项 | 删除该导入项拥有的生成数据与类型；被其他表/代码使用时给出具体依赖 |
| 表分片 | 重新生成受影响路由及分片集合，不能只删一个 JSON 留下旧目录记录 |
| 音频条目 | 不默认删除共享 Clip；删除 Clip 必须单独检查资源引用 |
| 动态资源入口 | 先检查逻辑名、生成引用、UI/音频/配置使用方和兼容别名；取消收录默认保留源文件 |
...
| 模块代码分包 | 使用平台允许的本地代码分包 | 本地随包代码，按需接入 | 默认 embedded，可选本地交付代码 Bundle |
...
这份自审覆盖新增合同与跨章节修改；当前交付仍为设计文档。平台时钟连续性、回调派发顺序和真实后台行为列为原型/设备验收，不写成已实现的保证。
...
coin.png 被奖励弹窗 Prefab 固定引用，又需要由道具配置选择显示。工作台中将它收录为 `battle/default/sprite/coin`，显示“有序列化引用 + 动态入口”两个标记，不复制图片。

配置单元格使用该逻辑 id，代码使用 `BattleRes.sprite.coin`，手动输入简称时明确 SpriteFrame 类型；它们解析到同一个实际资产。以后将图片移动到共享包，通过 AssetDB 保留 UUID，更新索引和构建依赖即可，既有逻辑名无需变化。
...
| P3 完整游戏基础 | UI 导航/缓存/Loading、音频、基础存储、章节包、服务器校时/日历订阅、平台生命周期 | 大厅→森林关卡→结算→大厅真实流程稳定运行；日切/月末与重新进入不重复结算，时间订阅正确退出 |
...
**原型 C：按需资源交付。** 一个远程资源包含两个互不依赖且没有共享合并文件的关卡。关闭整包压缩、清空缓存，只打开关卡 1，下载日志不出现关卡 2 独占文件。再访问关卡 2，确认新增请求。另设合并/Zip 场景记录预期差别。
...
**原型 F：目录与构建寻址。** 两个资源组各放同名图片，一个图片同时被 Prefab 静态引用，另有一个第三方图集。扫描生成名字后，在 Preview 和微信/原生构建中验证简称作用域、完整 id、类型化引用、子资源与图集帧。移动源文件保留 UUID 后名字不变；清空缓存只 resolve 名字时，仅读取允许的元数据；排除必需动态入口时构建检查必须失败。
...
| C02 | 一表按章节分片 | 包内仅对应行；全表主键、schema 与空分片行为正确 |
...
| C10 | 分片表未指定包、错包、查找其他章节主键 | 报目标/缺项错误，不扫描或下载其他分片 |
...
| A09 | 未读取章节索引时查名、再实际加载 | 默认按需方案只读必要配置/索引；实际加载才读取目标及其依赖，其他章节不提前访问 |
...
让不了解框架内部的新开发者，仅按模板完成：创建模块、创建弹窗、生成绑定、导入一张表、收录图标并按名字显示、处理一次重名、把章节放远程包、获取弹窗结果。
...
本文参考了当前仓库的 [历史架构设计](new-framework-design.md)、[工具面板说明](creator-tooling-panel-guide.md)，以及实际的模块装配、绑定生成、远程包校验、配置和资源生命周期代码。保留的是已证明存在的需求，不把历史实现视为新方案的约束。
