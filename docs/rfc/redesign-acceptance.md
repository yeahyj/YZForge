# 13. YZForge 重构验收标准（历史）

> 本文是 V1 验收记录。当前硬门禁见 [V2 验收标准](./v2-acceptance.md)。
...
- root `tsconfig.json` 是可提交、可迁移的项目契约：不设置已弃用的 `baseUrl`，`moduleResolution` 使用 `bundler`，`db://assets/*` 指向 `./assets/*`，`yzforge` 指向 `./packages/yzforge-runtime/src/index.ts`，不能 `extends` Cocos `temp` 配置，不能包含 `db://internal/*`，不能包含项目根绝对路径。
