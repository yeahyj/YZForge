...
export class ExtensionRegistry {
    ...
    private async runPhase(phase: ExtensionPhase): Promise<void> {
        const context = this.createContext(phase);
        for (const extension of this.sortInstalled()) {
            ...
            if (hook) {
                try {
                    ...
                    throw this.createPhaseError(extension, phase, error);
                }
    ...
    private createContext(phase: ExtensionPhase): ExtensionContext {
        return {
            app: this.app,
            ...
            logger: this.logger,
            phase,
            provide: (token, value) => this.provide(token, value),
            provideModule: (token, factory) => this.provideModule(token, factory),
        };
    }

    private createPhaseError(extension: Extension, phase: ExtensionPhase, cause: unknown): YZForgeError {
