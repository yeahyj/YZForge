...
test('calendar callbacks use project rules with explicit per-subscription overrides', async () => {
    const owner = new Scope('calendar-defaults'),
        clock = new FakeClock();
    clock.wall = ms('2026-01-01T23:00:00Z');
    const time = new TimeService(clock, owner, { calendar: { offsetMinutes: 120, weekStartsOn: 0, resetMinute: 240 } });
    const project = time.onBoundary('day', () => {}, owner),
        override = time.onBoundary('day', () => {}, owner, { offsetMinutes: 0, resetMinute: 0 });
...
test('business reset time participates in day, week, month and year keys', () => {
    const early = ms('2027-01-01T03:00:00+08:00'),
        rules = { offsetMinutes: 480, resetMinute: 240 };
...
test('local time queries return independent dates without 32-bit epoch truncation', async () => {
    const clock = new FakeClock(),
        owner = new Scope('test'),
        time = new TimeService(clock, owner);
    clock.wall = ms('2100-01-01T00:00:00Z');
    const date = time.nowDate();
    date.setTime(0);
    assert.equal(time.nowSeconds(), clock.wall / 1000);
    assert.equal(time.snapshot().quality, 'local');
    assert.throws(() => time.requireNowMs(), { code: 'TIME_NOT_SYNCED' });
    await owner.close();
    assert.equal(clock.timerCount, 0);
});
...
test('boundary coalesces missed days and does not repeat on backward corrections', async () => {
    ...
    await flush();
    clock.advance(86400000 * 4);
    await flush();
    assert.equal(seen.length, 1);
    assert.equal(seen[0].missedCount, 3);
    clock.jump(-86400000 * 3);
    clock.advance(60000);
    await flush();
    assert.equal(seen.length, 1);
    clock.advance(86400000 * 4);
...
test('background skips callbacks then checks latest eligible period on resume', async () => {
    ...
    await flush();
    clock.hide();
    clock.advance(86400000 * 20);
    await flush();
    assert.equal(seen.length, 0);
    clock.resume();
    await flush();
    assert.equal(seen.length, 1);
    assert.equal(seen[0].reason, 'resume');
    await owner.close();
});
...
test('monthly recurring targets always derive from original Jan 31 anchor', async () => {
    const clock = new FakeClock();
    clock.wall = ms('2026-01-31T08:00:00Z');
    const owner = new Scope('test'),
        time = new TimeService(clock, owner);
    const seen: any[] = [];
    const handle = time.everyPeriod(
        { unit: 'month', count: 1 },
        (event) => {
            seen.push(event);
        },
    ...
    clock.advance(ms('2026-02-28T08:00:00Z') - clock.wall);
    await flush();
    assert.equal(seen.length, 1);
    assert.equal(handle.nextAtMs, ms('2026-03-31T08:00:00Z'));
    await owner.close();
});
test('startup emitCurrent is asynchronous and no callbacks survive process owners', async () => {
    ...
    time.onBoundary(
        'day',
        (event) => {
            assert.equal(event.missedCount, null);
            assert.equal(event.reason, 'initial');
            calls++;
        },
        owner,
        { emitCurrent: true },
...
test('server sync accounts for server processing and extrapolates selected sample at commit', async () => {
    ...
    assert.equal(one.nowMs, origin + 1300);
    assert.equal(two.nowMs, one.nowMs);
    assert.equal(one.sampleAgeMs, 200);
    assert.equal(one.estimatedErrorMs, 21.01);
    clock.jump(-999999);
    assert.equal(time.nowMs(), origin + 1300);
    await owner.close();
});
...
test('cancelled/reset synchronization rounds cannot commit a late response', async () => {
    ...
    const pending = time.sync(caller);
    await flush();
    time.resetSync('logout');
    gate.resolve({ requestId: '1:0', receivedAtMs: clock.wall, sentAtMs: clock.wall });
    await assert.rejects(pending);
    assert.equal(time.snapshot().source, 'device');
    await owner.close();
});
