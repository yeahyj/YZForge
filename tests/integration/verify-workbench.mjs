...
assert.ok(
    !initial.modules.some((item) => item.id === module),
    'A previous test module exists; inspect it before retrying',
);
console.log(
    JSON.stringify(await action('createModule', { id: module, displayName: '工作台流程验证', codeOnly: true })),
);
let deleted = false;
try {
    const state = await editor('return await Editor.Message.request("yzforge-editor","state");');
    ...
    await editor('await Editor.Message.request("asset-db","save-asset",args.url,args.content);return true;', {
        url: service,
        content:
            "import {EntriesTable} from '../../generated/config/Entries.table'; export const referencedTable=EntriesTable;\n",
    });
    const generatedPath = `assets/game/modules/${module}/generated/config/Entries.table.ts`;
    const beforeRemoval = await editor(
        'return require("fs").readFileSync(require("path").join(Editor.Project.path,args.file),"utf8");',
        { file: generatedPath },
    );
    await action('removeTable', { id: `${module}.entries` });
    await assert.rejects(action('generate'), /旧生成文件仍被手写代码导入/);
    const afterRejection = await editor(
        'return require("fs").readFileSync(require("path").join(Editor.Project.path,args.file),"utf8");',
        { file: generatedPath },
    );
    ...
    const scriptPreview = await action('previewDelete', scriptArgs);
    assert.deepEqual(scriptPreview.references, []);
    await action('deleteModule', { ...scriptArgs, signature: scriptPreview.signature });
    const regenerated = await action('generate');
    assert.equal(regenerated.obsolete.length, 4);
    console.log(
        'PASS: obsolete configuration with business references is blocked before writes, then archived after references are removed',
    );
    const resolution = await editor('return await Editor.Profile.getProject("project","general.designResolution");');
    let view;
    try {
        await editor('await Editor.Profile.setProject("project","general.designResolution",args.value);return true;', {
            value: { ...resolution, width: 1280, height: 720 },
        });
    ...
    const size = (
        await call('execute_javascript', {
            context: 'scene',
            args: { uuid: view.uuid },
            code: 'const p=await new Promise((yes,no)=>cc.assetManager.loadAny(args.uuid,(error,value)=>error?no(error):yes(value)));const s=p.data.getComponent(cc.UITransform).contentSize;return {width:s.width,height:s.height};',
        })
    ).data.result;
    ...
    const before = await editor('return await Editor.Message.request("asset-db","query-asset-info",args.uuid);', {
        uuid: view.uuid,
    });
    const preview = await action('previewDelete', { module, kind: 'module' });
    assert.deepEqual(preview.references, []);
    const removed = await action('deleteModule', { module, kind: 'module', signature: preview.signature });
    deleted = true;
    await action('restore', { id: removed.restoreId });
    deleted = false;
    const after = await editor('return await Editor.Message.request("asset-db","query-asset-info",args.uuid);', {
        uuid: view.uuid,
    });
    ...
    console.error('Workbench verification failed:', error);
    throw error;
} finally {
    if (!deleted) {
        const preview = await action('previewDelete', { module, kind: 'module' });
