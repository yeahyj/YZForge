...
function chunk(name, content) { const type = Buffer.from(name), size = Buffer.alloc(4), checksum = Buffer.alloc(4); size.writeUInt32BE(content.length); checksum.writeUInt32BE(crc32(Buffer.concat([type, content]))); return Buffer.concat([size, type, content, checksum]); }
const width = 40, height = 40, pixels = Buffer.alloc((width * 4 + 1) * height);
for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
  const index = y * (width * 4 + 1) + 1 + x * 4, inside = (x - 19.5) ** 2 + (y - 19.5) ** 2 < 19 ** 2;
...
const rate = 22050, samples = Math.floor(rate * 0.18), wav = Buffer.alloc(44 + samples * 2);
...
for (let i = 0; i < samples; i++) wav.writeInt16LE(Math.round(Math.sin(i / rate * 2 * Math.PI * 660) * Math.sin(Math.PI * i / samples) * 4500), 44 + i * 2);
const result = await call('execute_javascript', { context: 'editor', args: { assets: [
