...
async function verify(name, code) {
  const data = await preview(`if(!app)throw Error('Bootstrap is not ready'); const check=(value,message)=>{if(!value)throw Error(message);}; const deadline=(pending)=>Promise.race([pending,new Promise((_,no)=>setTimeout(()=>no(Error('Integration deadline exceeded')),8000))]); ${code}`);
  assert.equal(data.ok, true, name); results.push({ name, ...data }); console.log(JSON.stringify({ name, ...data }));
}
await verify('automatic inherited binding, lazy bundle, JSON table and SpriteFrame', `
...
await verify('popup params/result and physical resource release', `
...
await verify('early finish in onShow never commits an interactive view', `
...
await verify('duplicate open rejected and one caller cancellation preserves existing UI', `
...
await verify('audio natural completion returns clip pin and pool source clears clip', `
...
await verify('a parent listener waiting on a child result drains when parent closes', `
...
await verify('page navigation from a tracked handler avoids self-wait and resumes its previous page', `
...
await verify('UI timeout isolates a module while retaining resources until physical cleanup', `
...
await verify('repeated popup closure unregisters long-lived owner cleanup callbacks', `
...
await verify('atlas frame leases retain the atlas and requested frame together', `
...
console.log(JSON.stringify({ passed: results.length, results, screenshot: await screenshot('yzforge-verified-dashboard.png') }, null, 2));
