...
async function csv(root, name, content) {
    await writeFile(join(root, `config-source/lobby/${name}.csv`), content);
}
async function mappings(root, tables) {
    await writeFile(join(root, 'config-source/tables.json'), JSON.stringify({ formatVersion: 1, tables }));
}
test('CSV supports embedded commas, newlines, quotes, BOM and trailing empty cells', () => {
    ...
    assert.throws(() => parseCSV('"closed"x'));
});
...
test('configuration generation shards rows, validates globally and keeps data out of TS', async () =>
    fixture(async (root, module) => {
        ...
        await mappings(root, [
            {
                id: 'lobby.levels',
                source: 'config-source/lobby/levels.csv',
                shards: { field: 'chapter', targets: { forest: 'forest', desert: 'desert' } },
                indexes: { chapter: { field: 'chapter', unique: false } },
            },
        ]);
        ...
        assert.equal(forest.rows.length, 1);
        assert.equal(forest.rows[0].enabled, false);
...
test('blank values use defaults then nullable, without treating false and zero as empty', async () =>
    fixture(async (root, module) => {
        ...
        const compiled = await compileTables(root, [module], runtime, new Map()),
            data = JSON.parse(compiled.output['assets/game/modules/lobby/res/config/items.json']);
...
test('foreign keys validate declared references rather than guessing from field names', async () =>
    fixture(async (root, module) => {
        ...
        await mappings(root, [
            { id: 'lobby.items', source: 'config-source/lobby/items.csv' },
            { id: 'lobby.levels', source: 'config-source/lobby/levels.csv' },
        ]);
...
test('XLSX import uses raw typed cells and rejects formulas with cell positions', async () =>
    fixture(async (root, module) => {
        const book = new ExcelJS.Workbook(),
            sheet = book.addWorksheet('Items');
        sheet.addRows([
            ['id', 'name', 'enabled'],
            ['int', 'string', 'bool'],
            [null, null, true],
        ...
        const file = join(root, 'config-source/lobby/items.xlsx');
        await book.xlsx.writeFile(file);
        await mappings(root, [{ id: 'lobby.items', source: 'config-source/lobby/items.xlsx', sheet: 'Items' }]);
        const compiled = await compileTables(root, [module], runtime, new Map());
        assert.equal(
            JSON.parse(compiled.output['assets/game/modules/lobby/res/config/items.json']).rows[0].name,
            '001',
        );
        sheet.getCell('A5').value = { formula: '1+1', result: 2 };
        await book.xlsx.writeFile(file);
...
test('dynamic asset fields produce typed logical keys without loading engine assets', async () =>
    fixture(async (root, module) => {
        ...
        assert.deepEqual(JSON.parse(result.output['assets/game/modules/lobby/res/config/items.json']).rows[0].icon, {
