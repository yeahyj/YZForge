...
async function rpc(method, params) {
  ...
  const response = await fetch(endpoint, { method: 'POST', headers,
    body: JSON.stringify({ jsonrpc: '2.0', id: ++sequence, method, params }), signal: AbortSignal.timeout(120000) });
  session = response.headers.get('mcp-session-id') ?? session;
  const raw = await response.text();
  const payload = JSON.parse(response.headers.get('content-type')?.includes('text/event-stream')
    ? raw.split(/\r?\n/).filter(line => line.startsWith('data:')).at(-1).slice(5) : raw);
...
export async function call(name, args = {}) {
  if (!verified) {
    await rpc('initialize', { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'yzforge-tools', version: '0.1.0' } });
    ...
    if (normalize(state.data.projectPath) !== normalize(root)) throw Error(`Wrong editor project: ${state.data.projectPath}`);
    verified = true;
  }
...
export async function schemas(pattern) {
  ...
  return result.tools.filter(tool => new RegExp(pattern).test(tool.name))
    .map(({ name, description, inputSchema }) => ({ name, description, inputSchema }));
}
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const [name, raw] = process.argv.slice(2);
  const args = raw?.startsWith('@') ? JSON.parse(readFileSync(resolve(raw.slice(1)), 'utf8')) : JSON.parse(raw ?? '{}');
  console.log(JSON.stringify(name === 'schemas' ? await schemas(args.pattern ?? '.') : await call(name, args), null, 2));
}
