...
import { Lifetime, scopeOwner } from '../core/scope';
import { ErrorReporter, reportError } from '../core/errors';
type Entry<V> = { users: number; value?: V; ready: Promise<V>; released: boolean; holders: Set<Lifetime> };

/**
 * @internal
 * 按键共享异步加载的租约缓存。每个 Lifetime 对同一键只有一份逻辑持有，多个 Lifetime 共享一次物理引用。
 * 最后一个持有者归还后释放；加载失败可重试，一个等待者取消不影响其他等待者。
 */
export class LeaseCache<V> {
    private readonly entries = new Map<string, Entry<V>>();
    private readonly owners = new WeakMap<Lifetime, Map<string, Promise<V>>>();
    /**
    ...
    constructor(
        ...
        private readonly release: (value: V) => void | Promise<void>,
     ...
     * @returns 共享加载结果；同一 Lifetime 重复取得同键不会重复持有。
     * @throws OperationCancelled scope 已取消或等待期间取消；加载失败原样传播。
     */
    acquire(key: string, scope: Lifetime): Promise<V> {
            ...
            captured.ready = Promise.resolve()
                ...
                .then(
                    async (value) => {
                        ...
                        if (captured.users === 0) await this.drop(key, captured);
                        return value;
                    },
                    (error) => {
        ...
        let returned = false,
            returning: Promise<void> | undefined,
            unown = () => {};
        const giveBack = () => {
            if (returned) return returning;
            returned = true;
            captured.holders.delete(scope);
            owned!.delete(key);
            unown();
            if (--captured.users === 0 && captured.value !== undefined) returning = this.drop(key, captured);
            return returning;
        };
        // Pending caller cancellation can return demand early. A delivered lease lives until cleanup.
        const off = scope.signal.onAbort(() => {
            if (captured.value === undefined) void giveBack();
        });
        unown = scope.defer(() => {
            off();
            return giveBack();
        });
        const result = untilCancelled(captured.ready, scope.signal).catch(async (error) => {
            off();
            await giveBack();
            throw error;
        });
    ...
    private async drop(key: string, entry: Entry<V>): Promise<void> {
        if (entry.released) return;
        entry.released = true;
        if (this.entries.get(key) === entry) this.entries.delete(key);
        try {
            await this.release(entry.value!);
        } catch (error) {
            this.report(error);
        }
    }
