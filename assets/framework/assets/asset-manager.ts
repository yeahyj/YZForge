import { Asset, assetManager, AudioClip, Font, ImageAsset, instantiate, isValid, JsonAsset, Material, Node, Prefab, SceneAsset, Sprite, SpriteAtlas, SpriteFrame, TextAsset, Texture2D, director, Director } from 'cc';
...
import { AssetAddress, AssetKey, AssetKind, AssetTypes, BundleRef, bundleId, ContentRelease, NamespaceIndex } from './asset-types';
import { logicalKey, resolveIndex, validateIndex } from './catalog';
import { LeaseCache } from './lease-cache';
const constructors = { Prefab, SpriteFrame, Texture2D, ImageAsset, AudioClip, JsonAsset, TextAsset, Material, SpriteAtlas, Font, SceneAsset };
...
export class Assets {
  ...
  constructor(readonly release: ContentRelease, owner: Scope) {
    invariant(!selectedRelease || selectedRelease === release.releaseId, 'RELEASE_RESTART_REQUIRED', 'A new content release requires a fresh JS runtime and engine asset cache');
    selectedRelease = release.releaseId;
    this.scope = owner.child('assets');
    this.validateBundles();
    this.cache = new LeaseCache(key => this.fetch(this.addresses.get(key)!), value => { value.atlas?.addRef(); value.asset.addRef(); }, value => { value.asset.decRef(); value.atlas?.decRef(); });
  }
  attachConfig(config: ConfigManager): void { this.config = config; }
  in(scope: Scope, namespace?: string): ScopedAssets { return new ScopedAssets(this, scope, namespace); }
  ...
  async resolve(keyOrName: AssetKey | string, typeOrScope: AssetKind | Scope, explicitScope?: Scope, namespace?: string): Promise<AssetAddress> {
    const key = typeof keyOrName === 'string' ? logicalKey(keyOrName, typeOrScope as AssetKind, namespace) : logicalKey(keyOrName.id, keyOrName.type);
    ...
    if (!pending) {
      ...
      pending = this.loadPath(route.bundle, route.path, 'JsonAsset', this.scope).then(asset => validateIndex(asset.json, ns));
      this.indices.set(ns, pending);
      void pending.catch(() => { if (this.indices.get(ns) === pending) this.indices.delete(ns); });
    }
  ...
  async loadPath<K extends AssetKind>(bundle: BundleRef, path: string, type: K, scope: Scope): Promise<AssetTypes[K]> {
    invariant(path.length > 0 && !path.split('/').includes('..') && !path.startsWith('/'), 'ASSET_PATH_INVALID', 'Use a bundle-relative asset path');
  ...
  async loadAddress<K extends AssetKind>(address: AssetAddress<K>, scope: Scope): Promise<AssetTypes[K]> {
    ...
    if (address.codeModule) invariant(this.codeReady(address.codeModule), 'MODULE_CODE_NOT_READY', `Load module code before deserializing ${address.path}`);
  ...
  async prepareBundle(id: string): Promise<EngineAssetManager.Bundle> {
    ...
    if (!pending) {
      ...
      pending = (async () => {
        ...
        return await new Promise<EngineAssetManager.Bundle>((resolve, reject) => assetManager.loadBundle(definition.location ?? id,
          definition.version ? { version: definition.version } : {}, (error, bundle) => error ? reject(error) : resolve(bundle)));
      })();
      this.bundles.set(id, pending);
      void pending.catch(() => { if (this.bundles.get(id) === pending) this.bundles.delete(id); });
    }
    return pending;
  }
  private async fetch(address: AssetAddress): Promise<LoadedAsset> {
    ...
    const asset = await new Promise<Asset>((resolve, reject) => bundle.load(address.path, type as typeof Asset, (error, value) => error ? reject(error) : resolve(value)));
    if (!address.atlasFrame) return { asset };
    const frame = (asset as SpriteAtlas).getSpriteFrame(address.atlasFrame);
    if (!frame) { asset.addRef(); asset.decRef(); }
  ...
  async instantiate(key: AssetKey<'Prefab'>, parent: Node, scope: Scope, input: { active?: boolean } = {}): Promise<Node> {
    const instance = scope.child(`instance:${key.id}`);
    try {
      const address = await this.resolve(key, instance);
      if (input.active !== false && address.codeModule) invariant(this.moduleReady(address.codeModule), 'MODULE_NOT_READY', `Module factory is still initializing: ${address.codeModule}`);
  ...
  activate(node: Node): void {
    ...
    invariant(instance && isValid(node, true), 'INSTANCE_NOT_MANAGED', 'Activate only a framework-owned prefab instance');
    ...
    this.activateInstance(node, instance.scope); node.active = true;
  }
  async setSprite(target: Sprite, key: AssetKey<'SpriteFrame'>, owner: Scope): Promise<void> {
    ...
    const pending = owner.child(`sprite:${key.id}`); state.pending = pending;
    try {
      const frame = await this.load(key, pending);
      pending.signal.throwIfAborted();
      if (state.sequence !== sequence || !isValid(target, true)) throw new FrameworkError('SPRITE_TARGET_CHANGED', 'Sprite assignment is no longer current');
      const previous = state.displayed;
      target.spriteFrame = frame; state.displayed = pending; state.pending = undefined;
      pending.defer(() => {
      ...
      await pending.close(); throw error;
    }
  }
  private validateBundles(): void {
    ...
    const visit = (id: string) => {
      ...
      invariant(this.release.bundles[id] && !visiting.has(id), 'BUNDLE_GRAPH_INVALID', `Missing or cyclic bundle: ${id}`);
      visiting.add(id);
...
export function destroyNode(node: Node): Promise<void> {
  ...
  return new Promise(resolve => {
    const check = () => { if (!isValid(node)) resolve(); else director.once(Director.EVENT_AFTER_DRAW, check); };
    director.once(Director.EVENT_AFTER_DRAW, check);
  });
}
export class ScopedAssets {
  constructor(readonly manager: Assets, readonly scope: Scope, readonly namespace?: string) {}
  in(scope: Scope): ScopedAssets { return new ScopedAssets(this.manager, scope, this.namespace); }
  ...
  resolve(key: AssetKey | string, type?: AssetKind): Promise<AssetAddress> {
    return typeof key === 'string' ? this.manager.resolve(key, type!, this.scope, this.namespace) : this.manager.resolve(key, this.scope);
  }
  ...
  load(key: AssetKey | string, type?: AssetKind): Promise<Asset> { return this.manager.load(typeof key === 'string' ? logicalKey(key, type!, this.namespace) : key, this.scope); }
  loadPath<K extends AssetKind>(bundle: BundleRef, path: string, type: K): Promise<AssetTypes[K]> { return this.manager.loadPath(bundle, path, type, this.scope); }
  openBundle(ref: BundleRef, owner = this.scope): Promise<BundleHandle> { return this.manager.openBundle(ref, owner); }
  instantiate(key: AssetKey<'Prefab'>, parent: Node, input?: { active?: boolean }): Promise<Node> { return this.manager.instantiate(key, parent, this.scope, input); }
  activate(node: Node): void { this.manager.activate(node); }
  setSprite(target: Sprite, key: AssetKey<'SpriteFrame'> | string): Promise<void> { return this.manager.setSprite(target, typeof key === 'string' ? logicalKey(key, 'SpriteFrame', this.namespace) as AssetKey<'SpriteFrame'> : key, this.scope); }
}
export class BundleHandle {
  readonly assets: ScopedAssets;
  readonly tables: ScopedConfig;
  constructor(manager: Assets, config: ConfigManager, readonly id: string, readonly scope: Scope) {
    this.assets = manager.in(scope, manager.release.bundles[id]?.namespace);
    this.tables = config.in(scope, id);
  }
  instantiate(key: AssetKey<'Prefab'>, parent: Node, input?: { active?: boolean }): Promise<Node> { return this.assets.instantiate(key, parent, input); }
}
