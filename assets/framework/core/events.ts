...
export interface EventKey<T> { readonly id: string; readonly __payload?: T; }
export function eventKey<T>(id: string): EventKey<T> { return Object.freeze({ id }); }
...
export class Events {
  ...
  on<T>(key: EventKey<T>, callback: (payload: T, task: TaskContext) => void | Promise<void>, scope: Scope): () => void {
    ...
    group.add(item);
    let detach = () => {};
    const off = () => { group!.delete(item); if (!group!.size) this.listeners.delete(key.id); detach(); };
  ...
  emit<T>(key: EventKey<T>, payload: T): void {
    for (const item of Array.from(this.listeners.get(key.id) ?? [])) {
      if (!item.scope.signal.aborted) void runTask(item.scope, task => item.invoke(payload, task))
