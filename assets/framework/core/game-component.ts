...
export class GameComponent extends Component {
  ...
  __bind(ctx: ModuleContext, instance: Scope, time: TimeService): void {
    ...
    this.ctx = ctx; this.instance = instance; this.time = time;
    instance.signal.onAbort(() => { this.allowed = false; void this.__deactivate().catch(reportError); });
    instance.defer(async () => { await this.__deactivate(); this.__dispose(); });
  }
  /** @internal An explicit business gate, separate from engine enabled/active. */
  __allow(owner: Scope | undefined): void {
    this.owner = owner; this.allowed = !!owner;
    if (!owner) { void this.__deactivate().catch(reportError); return; }
  ...
  onEnable(): void { if (this.allowed && !this.draining) this.activate(); }
  start(): void { /* Readiness is dispatched immediately before the first business frame. */ }
  update(dt: number): void {
    if (!this.activation || this.activation.signal.aborted) return;
    try {
      if (!this.ready) { this.ready = true; synchronous(this.onReady(), 'onReady'); }
      synchronous(this.onTick(dt), 'onTick');
    } catch (error) { this.allowed = false; void this.__deactivate().catch(reportError); reportError(error); }
  }
  lateUpdate(dt: number): void {
    if (!this.activation || this.activation.signal.aborted || !this.ready) return;
    try { synchronous(this.onLateTick(dt), 'onLateTick'); } catch (error) { this.allowed = false; void this.__deactivate().catch(reportError); reportError(error); }
  }
  onDisable(): void { void this.__deactivate().catch(reportError); }
  onDestroy(): void { void this.__deactivate().then(() => this.__dispose()).catch(reportError); }
  private activate(): void {
    ...
    const activation: ActivationContext = Object.freeze({ ...taskContext(scope, current), time: this.time!.in(scope),
      run: <T>(task: (context: TaskContext) => T | Promise<T>) => runTask(scope, task, current) });
    this.activation = activation;
    try { synchronous(this.onActivate(activation), 'onActivate'); }
    catch (error) { this.allowed = false; void this.__deactivate().catch(reportError); throw error; }
  }
  ...
  __deactivate(): Promise<void> {
    ...
    this.activation = undefined; activation.scope.cancel();
    let error: unknown;
    try { synchronous(this.onDeactivate(), 'onDeactivate'); } catch (failure) { error = failure; }
    this.draining = activation.scope.close().finally(() => { this.draining = undefined; });
  ...
  __dispose(): void {
    ...
    this.disposed = true; synchronous(this.onDispose(), 'onDispose');
  }
}
