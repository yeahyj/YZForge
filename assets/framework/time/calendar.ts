...
export interface CalendarPeriod { readonly unit: CalendarUnit; readonly count: number; }
export interface CalendarOptions { readonly offsetMinutes?: number; readonly weekStartsOn?: number; readonly resetMinute?: number; }
export interface DateParts {
  readonly year: number; readonly month: number; readonly day: number; readonly weekday: number;
...
export function validEpoch(value: number): number {
  invariant(Number.isSafeInteger(value) && value >= MIN_EPOCH_MS && value <= MAX_EPOCH_MS, 'INVALID_TIME', 'Expected UTC milliseconds in years 0001–9999');
  return value;
}
export function options(input: CalendarOptions = {}): Required<CalendarOptions> {
  const result = { offsetMinutes: input.offsetMinutes ?? 0, weekStartsOn: input.weekStartsOn ?? 1, resetMinute: input.resetMinute ?? 0 };
  invariant(Number.isInteger(result.offsetMinutes) && Math.abs(result.offsetMinutes) <= 840, 'INVALID_TIME_ZONE', 'offsetMinutes must be in [-840, 840]');
  invariant(Number.isInteger(result.weekStartsOn) && result.weekStartsOn >= 0 && result.weekStartsOn <= 6, 'INVALID_WEEK_START', 'weekStartsOn is 0 (Sunday) through 6');
  invariant(Number.isInteger(result.resetMinute) && result.resetMinute >= 0 && result.resetMinute < 1440, 'INVALID_RESET_TIME', 'resetMinute must be in [0, 1439]');
  return result;
}
export function validPeriod(period: CalendarPeriod, positive = false): void {
  invariant(units.includes(period.unit) && Number.isSafeInteger(period.count) && (!positive || period.count > 0), 'INVALID_CALENDAR_PERIOD', 'Invalid calendar unit or count');
}
...
function fromDate(date: Date): DateParts {
  return Object.freeze({ year: date.getUTCFullYear(), month: date.getUTCMonth() + 1, day: date.getUTCDate(),
    weekday: date.getUTCDay(), hour: date.getUTCHours(), minute: date.getUTCMinutes(), second: date.getUTCSeconds(), millisecond: date.getUTCMilliseconds() });
}
...
export function toISO(epochMs: number): string { return new Date(validEpoch(epochMs)).toISOString(); }
...
export function parseISO(text: string): number {
  ...
  invariant(year >= 1 && month >= 1 && month <= 12 && day >= 1 && day <= 31 && hour < 24 && minute < 60 && second < 60, 'INVALID_ISO_TIME', 'Invalid date or time fields');
  let offset = 0;
  if (m[8] !== 'Z') {
    const hours = Number(m[8].slice(1, 3)), minutes = Number(m[8].slice(4, 6));
    invariant(minutes < 60 && hours <= 14 && (hours < 14 || minutes === 0), 'INVALID_ISO_TIME', 'Invalid UTC offset');
  ...
  invariant(check.year === year && check.month === month && check.day === day, 'INVALID_ISO_TIME', 'Date does not exist');
...
export function add(epochMs: number, period: CalendarPeriod, offsetMinutes = 0): number {
  validPeriod(period);
  const p = parts(epochMs, offsetMinutes);
  if (period.unit === 'day' || period.unit === 'week') return validEpoch(epochMs + period.count * DAY * (period.unit === 'week' ? 7 : 1));
  const months = (p.year - 1) * 12 + p.month - 1 + period.count * (period.unit === 'year' ? 12 : 1);
  const year = Math.floor(months / 12) + 1, month = ((months % 12) + 12) % 12 + 1;
  ...
  return validEpoch(utc(year, month, Math.min(p.day, lastDay), p.hour, p.minute, p.second, p.millisecond) - offsetMinutes * 60000);
}
...
export function isSamePeriod(a: number, b: number, unit: CalendarUnit, input?: CalendarOptions): boolean { return startOf(a, unit, input) === startOf(b, unit, input); }
export function formatDuration(milliseconds: number): string {
  invariant(Number.isFinite(milliseconds), 'INVALID_DURATION', 'Duration must be finite');
  const seconds = Math.ceil(Math.max(0, milliseconds) / 1000);
  return `${pad(Math.floor(seconds / 3600))}:${pad(Math.floor(seconds / 60) % 60)}:${pad(seconds % 60)}`;
}
export const calendar = Object.freeze({ toISO, parseISO, parts, format, add, startOf, nextBoundary, periodKey, isSamePeriod,
  dayStartMs: (ms: number, input?: CalendarOptions) => startOf(ms, 'day', input),
  dayKey: (ms: number, input?: CalendarOptions) => periodKey(ms, 'day', input), formatDuration });
