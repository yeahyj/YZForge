...
export class Dashboard extends DashboardBinding {
  ...
  protected async onShow(show: ViewShowContext<DashboardParams, DashboardResult>): Promise<void> {
    ...
    show.time.onBoundary('day', event => { show.commit(() => { this.lblPeriod.string = `当前业务日：${event.occurrenceKey.split(':')[1]}\n本地时间 · 周一为每周开始`; }); }, { emitCurrent: true, offsetMinutes: 480 });
    show.listen(this.btnReward.node, Button.EventType.CLICK, async () => {
      if (this.popupBusy) return; this.popupBusy = true;
      try {
        const popup = await this.ctx.ui.open(LobbyViews.rewardPopup, { title: '每日体验奖励', amount: 100 }, show.scope);
        const result = await popup.result;
        show.commit(() => { this.lblResult.string = result.status === 'completed' && result.value.claimed ? `已领取 ${result.value.amount} 金币。\n弹窗任务和资源已完成清理。` : '本次未领取奖励，可以再次打开。'; });
      } finally { show.commit(() => { this.popupBusy = false; }); }
    });
    show.listen(this.btnReload.node, Button.EventType.CLICK, async () => { await this.readItems(show); show.commit(() => { this.lblResult.string = '配置读取完成。\n相同 Scope 内复用共享加载结果。'; }); });
    show.listen(this.btnAudio.node, Button.EventType.CLICK, async () => {
      ...
      await handle.ended;
      show.commit(() => { this.lblResult.string = '音效播放完成，播放句柄已结束。'; });
    });
