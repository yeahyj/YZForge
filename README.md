...
`create-yzforge` 只负责第一次创建项目，并且只会把 `extensions/yzforge` 带入新项目；模板维护者自用的其他 Cocos 插件不会被复制。进入项目后，创建 Module、View、配置表和升级框架，继续使用 Cocos 顶部菜单里的 `YZForge` 插件或 `npm run yzforge:*` 命令。
...
| `npm run yzforge:check` | 统一检查配置、生成物、严格架构和 TypeScript |
| `npm run yzforge:check:full` | 统一检查后追加完整 smoke |
